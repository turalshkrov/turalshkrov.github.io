// ============================================
//  md-loader.js
//  Fetches a .md file, parses frontmatter,
//  renders markdown, wires up code copy btns.
// ============================================
...
(function () {
  ...
  // ── Wrap code blocks — badge morphs into copy button on hover ────
  function enhanceCodeBlocks(container) {
    container.querySelectorAll('pre').forEach(pre => {
      ...
      badge.addEventListener('mouseenter', () => {
        if (!badge.classList.contains('copied')) badge.textContent = 'copy';
      });
  ...
  async function loadPost() {
    ...
    // Always resolve from origin root so the path is never relative to
    // /template/. Strips any accidental leading slash from the file value.
    // Result: https://turalshkrov.github.io/posts/writeups/thm/mr-robot/thm-mrrobot.md
    const fileUrl = window.location.origin + '/' + file.replace(/^\/+/, '');
    ...
    try {
      ...
      if (!res.ok) throw new Error(`HTTP ${res.status} &mdash; ${fileUrl}`);
      const raw = await res.text();

      const { meta, content } = parseFrontmatter(raw);

      if (titleEl && meta.title) titleEl.textContent = `${meta.title} — blog`;
